...
int DoBuyAndSell(const std::vector<std::vector<int>>& map, int profit, int nextbuy, int k, int last)
{
    int finalProfit = profit;
    if(k < last)
    {
        // map[i][j] = bought at index i, sold at index i + j + 1
        for (; nextbuy < map.size(); ++nextbuy) 
        {
            for (int nextsell = 0; nextsell < map[nextbuy].size(); ++nextsell) 
            {
                if(map[nextbuy][nextsell] > 0)
                {
                    int newProfit = DoBuyAndSell(map, profit + map[nextbuy][nextsell], nextbuy + nextsell + 2, k + 1, last) ;
                    if(newProfit > finalProfit)
...
int maxProfit_bruteforce(int k, const std::vector<int>& prices)
{
    int profit = 0;
    if(prices.size() >=2 && k > 0)
    {
        ...
        for (int i = 0; i < map.size(); ++i) 
        {
            for (int j = 0; j < map[i].size(); ++j) 
            {
                if (map[i][j] > 0) 
                {
                    int newProfit = DoBuyAndSell(map, map[i][j], i + j + 2, 1, k);
                    if(newProfit > profit)
                        profit = newProfit;
                }
            }
